...
class MessagesPage(BasePage):
    ...
    def _render_global_filters(self, query: Dict[str, Any]) -> str:
        """Render global filter controls using the centralized GlobalFilterSystem."""
        # Parse filter parameters
        filters = GlobalFilterSystem.parse_query_filters(query)

        # Get groups for dropdown
        groups = []
        for group in self.db.get_monitored_groups():
            groups.append({
                'group_id': group.group_id,
                'name': group.group_name or group.group_id
            })

        # Get senders based on group filter
        senders = None
        if filters.get('group_id'):
            users = self.db.get_group_members(filters['group_id'])
        else:
            users = self.db.get_all_users()

        # Format senders for the filter system
        if users:
            senders = []
            for user in users:
        ...
        # Render the filters using the global system
        return GlobalFilterSystem.render_filters(
            groups=groups,
            selected_group=filters.get('group_id'),
            selected_date=filters.get('date'),
            selected_hours=filters.get('hours'),
            selected_sender=filters.get('sender_id'),
            senders=senders,
            attachments_only=filters.get('attachments_only'),
    ...
    def _render_groups_tab(self, query: Dict[str, Any]) -> str:
        ...
        sender_filter = filters.get('sender_id')  # Note: changed from sender_uuid to sender_id
    ...
    def _render_summary_tab(self, query: Dict[str, Any]) -> str:
        ...
        return f"""
            <div class="summary-tab-content">
                ...
                <p>AI-powered summaries of conversations using global filters above</p>

                <div class="summary-actions" style="background: #f8f9fa; padding: 20px; border-radius: 8px; margin-bottom: 20px;">
                    <div style="display: flex; gap: 10px; align-items: center; flex-wrap: wrap;">
                        <button type="button" onclick="showSummaryPreview()" style="padding: 8px 16px; background: #6c757d; color: white; border: none; border-radius: 4px; cursor: pointer;">Preview</button>
                        <button type="button" onclick="generateSummary(false)" style="padding: 8px 16px; background: #007bff; color: white; border: none; border-radius: 4px; cursor: pointer;">Generate</button>
                        <span style="color: #666; font-size: 0.9em;">Select a group and date using the filters above, then use these buttons to generate summaries</span>
                    </div>
                </div>

                <!-- Preview Card -->
                <div id="summary-preview" class="card" style="display: none; margin-bottom: 20px; border-left: 4px solid #ffc107;">
                    <h3>Summary Preview</h3>
                    <div id="summary-preview-content"></div>
                </div>

                <!-- Results Card -->
                <div id="summary-results" class="card" style="display: none;">
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;">
                        <h3>Message Summary</h3>
                        <button onclick="generateSummary(true)" style="padding: 6px 12px; background: #28a745; color: white; border: none; border-radius: 4px; cursor: pointer; font-size: 0.9em;">Force Refresh</button>
                    </div>
