// AI Analysis JavaScript for Messages page - Unified interface for all AI analyses
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (typeSelect) {
        typeSelect.addEventListener('change', function() {
            updateMessageAnalysisDescription();
        });
    }
});

function updateMessageAnalysisDescription() {
    const typeSelect = document.getElementById('analysis-type-select');
    ...
    if (selectedId && messageAnalysisTypes[selectedId]) {
        descEl.textContent = messageAnalysisTypes[selectedId].description || '';
...
function showMessageAnalysisPreview() {
    ...
    const analysisType = messageAnalysisTypes[typeSelect.value];
    ...
    fetch(url)
        ...
        .then(data => {
            if (data.status === 'started' && data.job_id) {
                ...
                renderMessageAnalysisResults(data);
            } else {
...
function renderMessageAnalysisResults(data) {
    const contentDiv = document.getElementById('analysis-content');
    const titleEl = document.getElementById('analysis-title');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.addEventListener('keydown', function(e) {
        // Only work if we're on the AI Analysis tab
        const activeTab = document.querySelector('.tab-content.active');
        if (!activeTab || !activeTab.classList.contains('ai-analysis-tab-content')) {
            return;
        }
